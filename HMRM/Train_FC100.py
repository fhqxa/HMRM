...
from common.Tools import ContrastiveLoss_Hierarchical_Cost # HMRM Loss
from common.meter import Meter
from common.utils import detect_grad_nan, compute_accuracy, set_seed, setup_run
from models.dataloader.samplers import CategoriesSampler, CategoriesSampler_Hierarchial
from models.dataloader.data_utils import dataset_builder
from models.renet import RENet
from test_Hierarchical import test_main, evaluate	#多粒度
Alpha = 0.1
Beta = 1.0
Gamma = 1.1
ModelDir = '623_fc100'
...
def train(epoch, model, loader, optimizer, args=None):
    ...
    for i, ((data, train_labels, train_coarse_labels), (data_aux, train_labels_aux, train_coarse_labels_aux)) in enumerate(zip(tqdm_gen, train_loader_aux), 1):
        ...
        # HMRM loss
        fine_relations_max, fine_predcit = torch.max(logits, 1)
        logits_scores = F.softmax(logits, dim=1)
        logits_scores_max, max_index = torch.max(logits_scores, 1)
        contrastiveLoss_Hierarchical_Cost = ContrastiveLoss_Hierarchical_Cost(fine_predcit, label, train_coarse_labels, fine_relations_max, logits_scores_max, args.way, args.query, Gamma, probablity, theat)
        epi_loss.data = epi_loss.data * Beta + contrastiveLoss_Hierarchical_Cost_test8.item() * Alpha
...
def train_main(args):
    ...
    train_sampler = CategoriesSampler_Hierarchial(trainset.label, trainset.coarse_label, len(trainset.data) // args.batch, args.way, args.shot + args.query)# ‘//’：先除法，后向下取整
    train_loader = DataLoader(dataset = trainset, batch_sampler = train_sampler, num_workers=8, pin_memory = False) # pin_memory=True
    ...
    val_sampler = CategoriesSampler_Hierarchial(valset.label, valset.coarse_label, args.val_episode, args.way, args.shot + args.query)
