...
def parse_args(arg_mode):
    ...
    parser.add_argument('-dataset', type=str, default='cub',
                        choices=['miniimagenet', 'cub', 'tieredimagenet', 'cifar_fs', 'fc100'])
    parser.add_argument('-data_dir', type=str, default='datasets_path', help='dir of datasets')

    ''' about training specs '''
    #parser.add_argument('-batch', type=int, default=256, help='auxiliary batch size') # tiered 1-shot
    #parser.add_argument('-batch', type=int, default=128, help='auxiliary batch size') # mini
    parser.add_argument('-batch', type=int, default=64, help='auxiliary batch size')   # cub\cifar\fc100
    parser.add_argument('-temperature', type=float, default=0.2, metavar='tau', help='temperature for metric-based loss')
    # parser.add_argument('-lamb', type=float, default=0.25, metavar='lambda', help='loss balancing term') # mini,tiered
    parser.add_argument('-lamb', type=float, default=1.5, metavar='lambda', help='loss balancing term') # cub
    #parser.add_argument('-lamb', type=float, default=0.5, metavar='lambda', help='loss balancing term')  # cifar\fc100

    ''' about training schedules '''
    parser.add_argument('-max_epoch', type=int, default=60, help='max epoch to run') # 5-shot
    #parser.add_argument('-max_epoch', type=int, default=80, help='max epoch to run') # 1-shot
    #parser.add_argument('-max_epoch', type=int, default=100, help='max epoch to run') # 1-shot,tiered
    parser.add_argument('-lr', type=float, default=0.1, help='learning rate')
    parser.add_argument('-gamma', type=float, default=0.05, help='learning rate decay factor')
    parser.add_argument('-milestones', nargs='+', type=int, default=[40, 50], help='milestones for MultiStepLR') # 5-shot
    #parser.add_argument('-milestones', nargs='+', type=int, default=[60, 70], help='milestones for MultiStepLR') # 1-shot，cub,fc100,mini
    #parser.add_argument('-milestones', nargs='+', type=int, default=[60, 80], help='milestones for MultiStepLR') # 1-shot,tiered
    parser.add_argument('-save_all', default=False, action='store_true', help='save models on each epoch')
    ...
    ''' about CCA of RENet '''
    #parser.add_argument('-temperature_attn', type=float, default=5.0, metavar='gamma', help='temperature for softmax in computing cross-attention')# mini、cifar、tiered、fc100
    parser.add_argument('-temperature_attn', type=float, default=2.0, metavar='gamma',help='temperature for softmax in computing cross-attention') # cub
    ''' about env '''
    parser.add_argument('-gpu', default='0,1', help='the GPU ids e.g. \"0\", \"0,1\", \"0,1,2\", etc')
    parser.add_argument('-extra_dir', type=str, default='checkpoint_dir', help='extra dir name added to checkpoint dir')
